...
_TOOL_DEFS: list[dict] = [
    # ── internal / explore (show_to_user=False) ─────────────
    dict(name="search_relevant_tables", fn_key="search_relevant_tables",
         terminal=False, show_to_user=False, category="explore",
    ...
    dict(name="get_table_metadata", fn_key="get_table_metadata",
         terminal=False, show_to_user=False, category="explore",
         ...
         parameters={"type": "object",
             ...
             "required": ["table_name"]}),
    dict(name="get_table_sample_data", fn_key="get_table_sample_data",
         terminal=False, show_to_user=False, category="explore",
         description="获取单张表的 3 行样本数据，帮助理解数据格式和实际值。",
         parameters={"type": "object",
             "properties": {"table_name": {"type": "string", "description": "表名"}},
             "required": ["table_name"]}),
    dict(name="get_field_values", fn_key="get_field_values",
         terminal=False, show_to_user=False, category="explore",
    ...
    dict(name="load_skill", fn_key="load_skill",
         terminal=False, show_to_user=False, category="explore",
         ...
         parameters={"type": "object",
             ...
             "required": ["skill_id"]}),
    dict(name="execute_sql_query", fn_key="execute_sql_query",
         terminal=False, show_to_user=False, category="execute",
         description="执行已有的 SQL 查询并返回结果。结果会缓存到查询记录中。",
         parameters={"type": "object",
             "properties": {"record_id": {"type": "string", "description": "要执行的查询 record_id"}},
             "required": ["record_id"]}),
    dict(name="analyze_query_result", fn_key="analyze_query_result",
         ...
         parameters={"type": "object",
             ...
             "required": ["record_id"]}),
    dict(name="get_data_summary", fn_key="get_data_summary",
         terminal=False, show_to_user=False, category="explore",
         ...
         parameters={"type": "object",
             ...
             "required": ["record_id"]}),
    dict(name="get_data_preview", fn_key="get_data_preview",
         terminal=False, show_to_user=False, category="explore",
    ...
    dict(name="search_web", fn_key="search_web",
         terminal=False, show_to_user=False, category="explore",
    ...
    # ── internal SQL preview (show_to_user=False, terminal=False) ──
    dict(name="preview_sql", fn_key="preview_sql",
         terminal=False, show_to_user=False, category="internal",
         description="执行只读 SQL 查询进行内部数据验证（Agent 内部使用，不会展示给用户）。用此工具验证假设、检查数据分布、预览查询结果。需要最终呈现给用户时，用 create_sql_query。",
         parameters={"type": "object",
             "properties": {
    ...
    # ── user-facing create (show_to_user=True) ──────────────
    dict(name="create_chart", fn_key="create_chart",
         terminal=False, show_to_user=True, category="create",
    ...
    dict(name="create_sql_query", fn_key="create_sql_query",
         terminal=True, show_to_user=True, category="create",
         description="创建并验证一条新的 SQL 查询。成功后系统自动执行查询并生成图表展示给用户。仅用于最终答案——内部验证请用 preview_sql。验证 SQL 语法 → 检查引用的表是否已获取结构 → 确保只读。成功后返回 record_id。验证失败时返回具体错误——请修正后重新调用。",
         parameters={"type": "object",
             "properties": {"sql": {"type": "string", "description": "完整的 SQL SELECT 语句"}},
             "required": ["sql"]}),

    # ── user-facing edit (show_to_user=True) ────────────────
    dict(name="edit_sql_query", fn_key="edit_sql_query",
         terminal=True, show_to_user=True, category="edit",
    ...
    dict(name="edit_chart", fn_key="edit_chart",
         terminal=True, show_to_user=True, category="edit",
    ...
    dict(name="replace_sql_fragment", fn_key="replace_sql_fragment",
         terminal=True, show_to_user=True, category="edit",
    ...
    dict(name="ask_for_clarification", fn_key="ask_for_clarification",
         terminal=True, show_to_user=True, category="explore",
...
def register_all_tools() -> int:
    ...
    for td in _TOOL_DEFS:
        ...
        tool = ToolDef(
            ...
            show_to_user=td.get("show_to_user", False),
